...
export default function UnicornScene({
  ...
  useEffect(() => {
    ...
    const currentScriptId = scriptId.current;
    ...
    const initializeScene = async () => {
      ...
      const UnicornStudio = (window as Window & { UnicornStudio?: UnicornStudioType })
        .UnicornStudio;
      ...
      await UnicornStudio?.init({
        ...
        const ourScene = scenes.find(
          scene =>
            scene.element === elementRef.current || scene.element.contains(elementRef.current)
        );
        if (ourScene) {
    ...
    return () => {
      ...
      if (jsonFilePath) {
        const script = document.getElementById(currentScriptId);
        script?.remove();
      }
    };
